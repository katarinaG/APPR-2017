...
newdata <- data.frame('x'= 2018:2032)
newdata$y <- predict(fit1, newdata)
...
napoved1 <- ggplot(NULL) +
  ...
  geom_point(data = newdata, aes(x=x, y=y), size=3, color = "green") +
  scale_x_continuous(breaks = seq(2003, 2033, by =5)) + ggtitle("Napoved z uporabo linearne regresije") + 
  theme(plot.title = element_text(hjust = 0.5))


#Polinomska regresija
fit2 <- lm(stevilo ~ poly(leta,5), data = sodelujoci)
newdata2 <- data.frame('x'= 2018:2032)
newdata2$y <- predict(fit2, newdata2)

napoved2 <- ggplot(NULL) +
  geom_point(data = sodelujoci, aes(x=leta, y= stevilo), size=3, color = "blue") +
  xlab("Leto") + ylab("Število tekmovalcev") + theme_minimal()+ geom_line(data = sodelujoci, aes(x=leta, y= stevilo), size=1) +
  geom_point(data = newdata2, aes(x=x, y=y), size=3, color = "green") +
  scale_x_continuous(breaks = seq(2003, 2033, by =5)) + ggtitle("Napoved z uporabo polinomske regresije") + 
  theme(plot.title = element_text(hjust = 0.5))


#Eksponentni model
fit3 <- lm(log(stevilo) ~ leta, data = sodelujoci)
newdata3 <- data.frame('x'= 2018:2032)
newdata3$y <- predict(fit3, newdata3)

napoved3 <- ggplot(NULL) +
  geom_point(data = sodelujoci, aes(x=leta, y= stevilo), size=3, color = "blue") +
  xlab("Leto") + ylab("Število tekmovalcev") + theme_minimal()+ geom_line(data = sodelujoci, aes(x=leta, y= stevilo), size=1) +
  geom_point(data = newdata3, aes(x=x, y=y), size=3, color = "green") +
...
napovedi <- ggarrange(napoved1, napoved2, napoved3 + rremove("x.text"), ncol = 2, nrow = 2)
